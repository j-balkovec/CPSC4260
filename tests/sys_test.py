...
async def test_upload_empty_file(mocker) -> None:
    ...
    mock_read_text = mocker.patch("utils.utility._read_file_contents", return_value=mock_file_content)
...
async def test_clear_button_opens_confirmation_dialog() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        async def wait_for_dialog():
            for _ in range(10):  
                if any(isinstance(screen, ConfirmationDialog) for screen in app.screen_stack):
                    return False
                await asyncio.sleep(0.1)
            return True
...
async def test_exit_button_opens_confirmation_dialog() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert any(not isinstance(screen, ConfirmationDialog) for screen in app.screen_stack)
