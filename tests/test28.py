...
def get_euclidean_distance_v1(point1, point2):
    ...
    if not (
        len(point1) == len(point2)
        and all(isinstance(coord, (int, float)) for coord in point1 + point2)
    ):
        raise ValueError(
            "Points must be of the same dimension and contain numeric values."
        )
    squared_diffs = []
...
def find_distance_between_points_v1(p1, p2):
    ...
    if not (
        len(p1) == len(p2) and all(isinstance(coord, (int, float)) for coord in p1 + p2)
    ):
        raise ValueError(
            "Points must have the same dimensions and numeric coordinates."
        )
