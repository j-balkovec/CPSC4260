...
def audit_action_simple_v1(action, user):
    ...
    return (
        f"[AUDIT]: User {user} performed {action}"  # Similar to log_message_simple_v1
    )


def log_event_detailed_v2(
    event_type, details, timestamp=None, severity="MEDIUM", source="SYSTEM"
):
...
def audit_operation_complex_v2(
    operation_name,
    performed_by,
    details_data=None,
    event_time=None,
