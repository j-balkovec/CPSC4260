...
from core.constants import ERROR_CODES, YELLOW_TEXT, RESET_TEXT, RED_TEXT
...
class CustomExceptionSuper(Exception):
    ...
    def what(self) -> str:
        ...
        Returns:
            str: returns a formatted error message
        """
        ...
        return (
            ...
            + "[error]"
            + RESET_TEXT
            + f": {self.error_name.lower()} occurred"
            + YELLOW_TEXT
            + "\n[error code]"
            + RESET_TEXT
            + f": {ERROR_CODES.get(self.error_key, 'N/A')}"
            + YELLOW_TEXT
            + "\n[message]"
            + RESET_TEXT
            + f": {self.args[0]}"
