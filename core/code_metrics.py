...
code_metrics_logger = setup_logger(
    name="code_metrics.py_logger", log_file="code_metrics.log"
)
# ==========
...
def _classify_lines_of_code(source_code: str) -> dict:
    ...
    grouped = {"comments": [], "blank_lines": [], "code": []}
    ...
    for line_number, line in enumerate(source_code.splitlines(), start=1):
        ...
        if in_multiline_comment:
            ...
            code_metrics_logger.debug(
                f"[Line {line_number}] Inside multi-line comment."
            )
            ...
            if multiline_delimiter in stripped_line:
                ...
                multiline_delimiter = None
                code_metrics_logger.debug(
                    f"[Line {line_number}] Exiting multi-line comment."
                )
            continue
        ...
        if any(delim in stripped_line for delim in ('"""', "'''")):
            ...
            if (stripped_line.count('"""') == 2) or (stripped_line.count("'''") == 2):
                code_metrics_logger.debug(
                    f"[Line {line_number}] Complete multi-line comment on same line."
                )
                in_multiline_comment = False
                multiline_delimiter = None
            else:
    ...
    code_metrics_logger.info(
        f"Line classification completed: "
...
def _calculate_code_metrics(info_dict: dict) -> dict:
    ...
    if metrics["SLOC"] > 0:
        metrics["Comment Density"] = round(
            len(info_dict["comments"]) / metrics["SLOC"], ndigits=3
        )
        metrics["Blank Line Density"] = round(
            len(info_dict["blank_lines"]) / metrics["SLOC"], ndigits=3
        )
